...
func getPrefixListFromCache(c *gin.Context) {
	...
	isLegacyAsSet, _ := regexp.MatchString("AS-\\w{1,32}$", asnOrAsSet)
	isModernAsSet, _ := regexp.MatchString("AS\\d{1,6}:\\w{1,32}$", asnOrAsSet)
	isAristaAsSet, _ := regexp.MatchString("AS\\d{1,6}_\\w{1,32}", asnOrAsSet)
